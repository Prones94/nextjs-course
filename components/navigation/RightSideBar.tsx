      ...
      <div>
        ...
        <div className="mt-7 flex w-full flex-col gap-[30px]">
          { hotQuestions.map(({_id, title}) => (
            <Link key={_id} href={ROUTES.PROFILE(_id)} className="cursor-pointer items-center justify-between gap-7">
              <p className='body-medium text-dark500_light700'>{title}</p>

              <Image src="/public/icons/chevron-right.svg" alt="Chevron" width={20} height={20} className="invert-colors"/>
            </Link>
          ))}
        </div>
      </div>
